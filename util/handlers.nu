...
(function generate-handlers-for-type (type count)
    ...
    (eval (NuTemplate codeForString:<<-END
                                       ...
                                       { 
                                      _    va_list ap; 
                                      _    va_start(ap, selector); 
                                      _    <% (if (!= type "void") %><%= type %> result;<% ) %>
                                      _    nu_handler(<% (if (!= type "void") then %>&result<% else %>0<% ) %>, &nu_handlers_returning_<%= lower %>[<%= i %>], receiver, ap); 
                                      _    <% (if (!= type "void") %>return result;<% ) %>
                                       }
                                       <% )) %>
                                       void nu_init_handlers_returning_<%= lower %>() {<% (count times:(do (i) %>
                                      _    nu_handlers_returning_<%= lower %>[<%= i %>].handler = (IMP) nu_handler_returning_<%= lower %>_<%= i %>;<% )) %>
                                       }
                                       END)))
